...
- **THEN** it MUST build the real `cmd/facter` binary and verify the Windows release-gate fact set (including OS, system32, networking, memory, processors, DMI, uptime, virtualization, FIPS, and timezone) through the CLI using structured fact names only
