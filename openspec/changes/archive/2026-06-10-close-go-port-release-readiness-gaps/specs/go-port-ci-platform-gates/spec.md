...
- **THEN** the unit-test workflow job MUST fail, and the gate MUST be documented in `PORTING.md` as a release-blocking check

#### Scenario: Gate covers the Windows release fact set
- **WHEN** the Windows release gate runs
- **THEN** it MUST build the real `cmd/facter` binary and verify the Windows release-gate fact set (including OS, system32, networking, memory, processors, DMI, uptime, virtualization, FIPS, timezone, and legacy aliases) through the CLI
...
- **THEN** it MUST build linux, darwin, windows, and freebsd targets only, and MUST NOT build openbsd, netbsd, dragonfly, solaris, or aix targets
