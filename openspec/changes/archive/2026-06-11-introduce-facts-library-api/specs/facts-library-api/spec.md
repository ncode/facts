...
`facts.New` SHALL return an Engine that resolves core facts only: it MUST NOT read `facter.conf`, scan custom or external fact directories, execute external-fact scripts, read external-fact environment variables, or touch the persistent cache unless the corresponding functional option is supplied. A `WithSystemDefaults` option SHALL configure an Engine with full CLI-equivalent system-following behavior.

#### Scenario: Default engine performs no implicit host configuration
- **WHEN** a consumer calls `facts.New()` with no options on a host that has a `facter.conf`, populated default fact directories, and executable external facts
- **THEN** discovery resolves core facts only, executes no scripts, reads no config file, and the resolved facts are unaffected by the host's facter configuration

#### Scenario: Explicit opt-in to fact sources
- **WHEN** an Engine is constructed with `WithCustomDirs`, `WithExternalDirs`, `WithConfigFile`, or `WithFact` options
- **THEN** discovery loads exactly the opted-in sources with input-contract semantics (DSL parsing, weights, confines, precedence, `facter.conf` interpretation) identical to the CLI's

#### Scenario: System-following engine matches CLI behavior
- **WHEN** an Engine is constructed with `WithSystemDefaults`
- **THEN** its resolved canonical tree matches what the `facter` CLI resolves on the same host
...
- **WHEN** two Engines with different configurations (e.g. different custom facts registered for the same fact name) discover concurrently in one process
...
- **WHEN** a consumer decodes a core, custom, or external fact subtree into a matching struct type via `facts.As[T]`
- **THEN** the populated value reflects the canonical tree, regardless of which source (core, custom, external) won precedence
...
- **WHEN** a consumer queries a name no fact resolved (`ErrFactNotFound` case) and separately queries a custom fact that legitimately resolved to nil
...
- **WHEN** an Engine constructed without `WithLogger` encounters warn-class conditions (e.g. an invalid custom-fact file in an opted-in directory)
