...
- `currentWindowsDMI` (dmi.go:415): fold into the dmi pair-merge (it needs `s.logr()`, same pattern) or exclude with a recorded reason — decide during the session-io dmi task; either outcome is acceptable, silence is not.
- plan9 guest availability at archive time determines smoke-vs-accepted-risk for the `path` fact check.
