...
- ~~plan9 guest availability at archive time determines smoke-vs-accepted-risk for the `path` fact check.~~ **Resolved (accepted risk):** the nlab Windows and Plan 9 guests were unavailable in the implementation environment, so the windows env-casing and plan9 `path` paths are verified guest-independently instead — `GOOS=windows` and `GOOS=plan9` cross-builds pass, and the env/casing and assembly logic is exercised by fake-host unit tests (the `envValue` windows/unix/plan9 casing test, the fake-windows `sshCoreFacts`/`osCoreFacts` assembly tests, and the existing plan9 identity/path tests). Real-guest smokes remain a pre-archive gate to run when nlab is reachable.
